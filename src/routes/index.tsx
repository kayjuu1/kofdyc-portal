...
import {getDocumentDownloadUrl, getDocuments} from "@/functions/documents"
...
import {seo, seoLinks, SITE_URL, DEFAULT_DESCRIPTION} from "@/lib/seo"
...
export const Route = createFileRoute("/")({
    ...
    head: () => ({
        meta: seo({
            title: "KOFDYC — Koforidua Diocesan Youth Council",
            path: "/",
        }),
        links: seoLinks("/"),
    }),
...
const ORGANIZATION_JSONLD = JSON.stringify({
    ...
    "@type": "Organization",
    name: "Koforidua Diocesan Youth Council",
    alternateName: "KOFDYC",
    url: SITE_URL,
    logo: `${SITE_URL}/icon-512.png`,
    description: DEFAULT_DESCRIPTION,
    parentOrganization: {
        "@type": "Organization",
        name: "Catholic Diocese of Koforidua",
    },
})
...
function DarkHero({
    ...
    return (
        <section className="relative flex w-full items-center justify-center overflow-hidden bg-slate-900">
            ...
            <div
                aria-hidden="true"
                className="absolute inset-0 bg-linear-to-t from-slate-950/90 via-slate-950/60 to-slate-950/50"
            />
...
function FeaturedSection({
    ...
    return (
        <section className="border-b border-border/40 bg-background">
            ...
            <span
                className="inline-block rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-primary">
              Featured
            </span>
                        <ScopeFilter/>
                    </div>
                            ...
                            >
                                ...
                                <div className="relative mb-5 overflow-hidden rounded-xl bg-muted">
                                    {article.coverImageUrl ? (
                                        ...
                                        <div
                                            className="flex aspect-2/1 w-full items-center justify-center bg-gradient-to-br from-primary/5 to-primary/10">
                                            <ImageIcon className="size-12 text-primary/20"/>
                                        </div>
                                    )}
                                    <div className="absolute left-3 top-3 flex gap-1.5">
                                        <Badge
                                            className="border-0 bg-background/90 text-xs capitalize shadow-sm backdrop-blur-sm">
                                        ...
                                        {events.slice(0, 3).map((event) => {
                                            ...
                                            return (
                                                <div key={event.id} className="flex gap-3">
                                                    <div
                                                        className="flex size-11 shrink-0 flex-col items-center justify-center rounded-lg bg-primary/10 text-primary">
                                                        <span
                                                            className="text-[10px] font-semibold uppercase">{month}</span>
        ...
        <section id="news" className="py-16 sm:py-20">
            <div className="mx-auto max-w-7xl px-4 sm:px-6">
                <div className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
            <span
                className="mb-2 inline-block rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-primary">
              Latest
            </span>
...
function AboutSection() {
    return (
        <section id="about" className="border-t border-border/40 bg-muted/20 py-16 sm:py-20">
          ...
          <span
              className="mb-3 inline-block rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-primary">
            About DYC
          </span>
